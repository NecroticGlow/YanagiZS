...
mod handlers {
    ...
    pub async fn on_rpc_enter_world_arg(
        ctx: &mut NetworkContext<'_, RpcEnterWorldArg>,
    ) -> Result<RpcEnterWorldRet, Retcode> {
    ...
    pub async fn on_rpc_post_enter_world_arg(
        _ctx: &mut NetworkContext<'_, RpcPostEnterWorldArg>,
    ) -> Result<RpcPostEnterWorldRet, Retcode> {
        Ok(RpcPostEnterWorldRet::default())
    }

    pub async fn on_rpc_scene_transition_arg(
        _ctx: &mut NetworkContext<'_, RpcSceneTransitionArg>,
    ) -> Result<RpcSceneTransitionRet, Retcode> {
        Ok(RpcSceneTransitionRet::default())
    }

    pub async fn on_rpc_enter_section_complete_arg(
        _ctx: &mut NetworkContext<'_, RpcEnterSectionCompleteArg>,
    ) -> Result<RpcEnterSectionCompleteRet, Retcode> {
        Ok(RpcEnterSectionCompleteRet::default())
    }

    pub async fn on_rpc_save_pos_in_main_city_arg(
        ctx: &mut NetworkContext<'_, RpcSavePosInMainCityArg>,
    ) -> Result<RpcSavePosInMainCityRet, Retcode> {
    ...
    pub async fn on_rpc_enter_section_arg(
        ctx: &mut NetworkContext<'_, RpcEnterSectionArg>,
    ) -> Result<RpcEnterSectionRet, Retcode> {
        let player_info = &mut ctx.session.player_info;
        ...
        *section_id = ctx.arg.section_id as i32;
        ...
        let mut scene_info =
            build_client_scene_info(&ctx.session.player_info, cur_scene_uid).unwrap();
        scene_section_util::add_scene_units_to_scene_info(
            ctx.session,
            cur_scene_uid,
            &mut scene_info,
        );
    ...
    pub async fn on_rpc_refresh_section_arg(
        _ctx: &mut NetworkContext<'_, RpcRefreshSectionArg>,
    ) -> Result<RpcRefreshSectionRet, Retcode> {
    ...
    pub async fn on_rpc_begin_training_course_battle_arg(
        ctx: &mut NetworkContext<'_, RpcBeginTrainingCourseBattleArg>,
    ) -> Result<RpcBeginTrainingCourseBattleRet, Retcode> {
        let player_info = &mut ctx.session.player_info;
    ...
    pub async fn on_rpc_battle_report_arg(
        _ctx: &mut NetworkContext<'_, RpcBattleReportArg>,
    ) -> Result<RpcBattleReportRet, Retcode> {
        Ok(RpcBattleReportRet::default())
    }

    pub async fn on_rpc_end_battle_arg(
        _ctx: &mut NetworkContext<'_, RpcEndBattleArg>,
    ) -> Result<RpcEndBattleRet, Retcode> {
        Ok(RpcEndBattleRet::default())
    }

    pub async fn on_rpc_leave_cur_scene_arg(
        ctx: &mut NetworkContext<'_, RpcLeaveCurSceneArg>,
    ) -> Result<RpcLeaveCurSceneRet, Retcode> {
        let scene_uid = *ctx
            .session
            .player_info
            .dungeon_collection()
